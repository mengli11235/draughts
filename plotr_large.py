"""auxiliary files to plot results against the random player and Alpha-Beta player"""
...
def loadLog(args):
    ...
    filename = os.path.join(folder, "mat_plot_rp_large.dat")
    if os.path.isfile(filename):
...
if __name__=="__main__":
    ...
    i, three_arr, mvp_three, iter_arr = loadLog(args3)
    nnet3 = {}
    n3 = nn(g,args3)
    nnet3['n1'] = n3
    args3.stage = 1
    ...
    ap = AlphaBetaDraughtsPlayer(g,2).play
    #ap = RandomPlayer(g).play
    index0 = len(filename_list[0]) if len(filename_list[0]) < 50 else 50
    for k in range(start_i, index0):
        if k < len(filename_list[0]):
            nnet3['n1'].load_checkpoint(folder=args3.checkpoint, filename=getCheckpointFile(filename_list[0][k]))
            nnet3['s2'].load_checkpoint(folder=args3.checkpoint, filename=getCheckpointFile(filename_list[0][k], False, 1))
            mcts3 = MCTS(g, nnet3, args3)
            n3p = lambda v,w,x,y,z: np.argmax(mcts3.getActionProb(v,w,x,y,z, temp=0))
            arena = Arena.Arena(ap, n3p, g)
        ...
        if k < len(filename_list[1]):
          nnet4['n1'].load_checkpoint(folder=args4.checkpoint, filename=getCheckpointFile(filename_list[1][k]))
          nnet4['s1'].load_checkpoint(folder=args4.checkpoint, filename=getCheckpointFile(filename_list[1][k], True))
          nnet4['s2'].load_checkpoint(folder=args4.checkpoint, filename=getCheckpointFile(filename_list[1][k], False, 1))
          nnet4['s3'].load_checkpoint(folder=args4.checkpoint, filename=getCheckpointFile(filename_list[1][k], False, 2))
          mcts4 = MCTS(g, nnet4, args4)
          n4p = lambda v,w,x,y,z: np.argmax(mcts4.getActionProb(v,w,x,y,z, temp=0))
          arena = Arena.Arena(ap, n4p, g)
    ...
    plt.savefig(args.checkpoint +"/"+str(i) + "rpEloScores_large.jpg")
    plt.close()