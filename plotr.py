...
def loadLog(args):
    ...
    filename = os.path.join(folder, "mat_plot_rp.dat")
    if os.path.isfile(filename):
        with open(filename, "rb") as f:
            print("Plot history found. Read from:")
            temp = Unpickler(f).load()
            [baseline_arr, mvp_arr,three_arr, mvp_three, iter_arr] = temp
            print(iter_arr[-1])
        f.closed
    return iteration, baseline_arr, mvp_arr, three_arr, mvp_three, iter_arr
...
if __name__=="__main__":
    ...
    i, baseline_arr, mvp_arr, three_arr, mvp_three, iter_arr = loadLog(args)
    g = DraughtsGame(10)
    n1 = nn(g,args)
    nnet1 = {}
    nnet1['n1'] = n1
    ...
    elo_scores_baseline = baseline_arr[-1] if len(baseline_arr) > 0 else 0
    ...
    for k in range(start_i, index0):
        ...
        arena = Arena.Arena(ap, n1p, g)
        _, n0wins, draws0 = arena.playGames(args.arenaCompare)
        elo_scores_baseline = calculate_elo(n0wins+draws0*0.5, args.arenaCompare, elo_scores_baseline, 0)
        baseline_arr.append(elo_scores_baseline)
        ...
        iter_arr.append(k)
        saveArr([baseline_arr, mvp_arr,three_arr,mvp_three,iter_arr])
    index1 = len(baseline_arr) if len(baseline_arr) < 50 else 50
    ...
    plt.plot(baseline_arr[0:index1],label="baseline")     
    ...
    plt.savefig(args.checkpoint +"/"+str(i) + "rpEloScores.jpg")
    plt.close()